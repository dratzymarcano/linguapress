...
class TRP_Machine_Translator {
    ...
    public function automatic_translate_error_check( $machine_translator, $translation_engine, $api_key ) {
        ...
        switch ( $translation_engine ) {
            ...
            case 'openrouter':
                if ( empty( $api_key ) ) {
                    $is_error = true;
                    $return_message = __( 'Please enter your OpenRouter API key.', 'translatepress-multilingual' );
                } else {
                    // Perform test.
                    $response = $machine_translator->test_request();
                    $code     = wp_remote_retrieve_response_code( $response );
                    if ( 200 !== $code ) {
                        $is_error        = true;
                        $return_message     = __( 'There was an error accessing the OpenRouter API.', 'translatepress-multilingual' );
                    }
                }
                break;
            case 'chatgpt':
                if ( empty( $api_key ) ) {
                    $is_error = true;
                    $return_message = __( 'Please enter your ChatGPT API key.', 'translatepress-multilingual' );
                } else {
                    // Perform test.
                    $response = $machine_translator->test_request();
                    $code     = wp_remote_retrieve_response_code( $response );
                    if ( 200 !== $code ) {
                        $is_error        = true;
                        $return_message     = __( 'There was an error accessing the OpenAI API.', 'translatepress-multilingual' );
                    }
                }
                break;
            default:
    ...
    public function get_api_key(){
        ...
        if( !empty( $this->settings['trp_machine_translation_settings']['translation-engine'] ) ){
            switch( $this->settings['trp_machine_translation_settings']['translation-engine'] ){
                case 'google_translate_v2':
                    if( !empty( $this->settings['trp_machine_translation_settings']['google-translate-key'] ) )
                        $key = $this->settings['trp_machine_translation_settings']['google-translate-key'];
                    break;
                case 'deepl':
                    if( !empty( $this->settings['trp_machine_translation_settings']['deepl-api-key'] ) )
                        $key = $this->settings['trp_machine_translation_settings']['deepl-api-key'];
                    break;
                case 'openrouter':
                    if( !empty( $this->settings['trp_machine_translation_settings']['openrouter-api-key'] ) )
                        $key = $this->settings['trp_machine_translation_settings']['openrouter-api-key'];
                    break;
                case 'chatgpt':
                    if( !empty( $this->settings['trp_machine_translation_settings']['chatgpt-api-key'] ) )
                        $key = $this->settings['trp_machine_translation_settings']['chatgpt-api-key'];
                    break;
            }
        }
        return $key;
